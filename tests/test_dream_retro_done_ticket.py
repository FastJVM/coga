...
def _seed_repo(root: Path, *, task_status: str = "done") -> Path:
...
def test_done_ticket_apply_appends_report_and_deletes_task(tmp_path: Path) -> None:
    ...
    assert "`relay-os/skills/code/implement-and-pr/SKILL.md`" in text
    assert "`relay-os/workflows/code/with-review.md`" in text

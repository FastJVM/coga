...
def test_lifecycle(seeded: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert cfg.projects["email-tool"].path is not None
    assert cfg.projects["email-tool"].path.is_dir()
    ...
    # 3. Advance steps.
    runner = CliRunner()
    r = runner.invoke(app, ["step", "2", "--task", "email-tool/001"])
    assert r.exit_code == 0, r.output
    r = runner.invoke(app, ["step", "3", "--task", "email-tool/001"])
    assert r.exit_code == 0
    r = runner.invoke(app, ["step", "4", "--task", "email-tool/001"])
    assert r.exit_code == 0
    r = runner.invoke(app, ["step", "5", "--task", "email-tool/001"])
    assert r.exit_code == 0, r.output

...
def test_ticket_state_guard_allows_exact_unlaunched_start_rollback(
    git_repo,
) -> None:
    """The one backward status write is pinned to the complete input ticket."""
    cfg = load_config(git_repo.coga_os)
    ticket = _seed_demo_ticket(
        git_repo,
        status="in_progress",
        blackboard="peer body edit survives\n",
    )
    expected = ticket.read_bytes()
    ticket.write_text(
        _step_ticket_text(
            step="1 (implement)",
            status="active",
    ...
    git.sync_task_state(
        cfg,
        ticket.parent,
        message="Ticket: demo — active after refused start",
        guard=git.ticket_state_guard(
            cfg,
            ticket,
            expected_lifecycle=("in_progress", "1 (implement)", "claude"),
            allow_status_regression=True,
            expected_ticket_bytes=expected,
        ),
    ...
    assert "status: active" in control
    assert "peer body edit survives" in control


def test_exact_unlaunched_start_rollback_refuses_newer_control_body(
    git_repo,
) -> None:
    """A second same-lifecycle peer edit wins the rollback publication race."""
    cfg = load_config(git_repo.coga_os)
    ticket = _seed_demo_ticket(
        git_repo,
        status="in_progress",
        blackboard="first peer body\n",
    )
    expected = ticket.read_bytes()
    git_repo.push_competing_commit(
        "coga/tasks/demo/ticket.md",
    ...
    with pytest.raises(
        git.StateRegressionError,
        match="exact control ticket changed",
    ):
        git.sync_task_state(
            cfg,
            ticket.parent,
    ...
    assert "newer peer body" in control
    assert "first peer body" not in control
...
@pytest.mark.parametrize("checkout_kind", ["feature", "detached"])
def test_bump_rewind_no_remote_still_guards_local_control_status(
    git_repo, checkout_kind: str
) -> None:
    """A missing remote cannot bypass rewind equality in another checkout."""
    ticket = _seed_demo_ticket(
        git_repo,
        status="paused",
        blackboard="stale local notes\n",
    ...
    if checkout_kind == "feature":
        git_repo.checkout_branch("feature/rewind-no-remote")
    else:
        git_repo.git("checkout", "--detach")
    before_head = git_repo.git("rev-parse", "HEAD").strip()
    ...
    assert "step: 1 (implement)" in control_ticket
    assert "newer blocker notes" in control_ticket
    local_ticket = ticket.read_text()
    assert "status: paused" in local_ticket
...
def test_detached_rewind_failed_landing_unwinds_scoped_commit(git_repo) -> None:
    """An unpublished detached rewind stays dirty after a transport failure."""
    ticket = _seed_demo_ticket(
        git_repo,
        status="paused",
        blackboard="local notes\n",
    ...
    git_repo.git(
        ...
        "set-url",
        "origin",
        str(git_repo.origin.parent / "gone.git"),
    )

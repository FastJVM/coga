#!/usr/bin/env python3
"""Utility maintenance-fee window sweep — harness-backed retrofit.

Behaviourally identical stdout to the standalone ``golden/maintenance_fee_sweep.py``
for the same ``--today`` / ``--tasks-dir``: the CLI harness and the alert
posting now come from ``coga.reminders.run``. Everything else — the frontmatter
reader, the grant-anchored window math, the ``patent_maintenance_paid == N``
satisfied rule, and the report/message formats — stays here, because it is
this sweep's own contract.

This is the **date window** worked example (auto-detect ``satisfied()``). The
live patents migration is a downstream follow-up (this PR only vendors it as a
...
def _read_frontmatter(path: Path) -> dict[str, str]:
    ...
    Deliberately dependency-free and string-typed, matching the standalone
    original: ``coga.ticket.Ticket.parse`` is YAML-typed (a date becomes a
    ``date``, ``1`` an ``int``) and raises on a fence-less file, and the report
    prints raw field text. We read the block between the first two ``---``
    fences and keep only column-0 ``key: value`` lines, so nested/indented keys
    ...
    for path in sorted(tasks_dir.rglob("*.md")):
        ...
        records.append(
            PatentRecord(
                ...
                number=number,
                granted=_parse_date(fm.get("patent_granted", "")),

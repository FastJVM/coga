#!/usr/bin/env python3
"""Candidate first-office-action reminder sweep — harness-backed retrofit.
...
posting now come from ``coga.reminders.run``. The frontmatter reader, the
candidate-stage selector, the single ``[filing+14mo, filing+15mo)`` window and
its month arithmetic, the four evaluate buckets, and the report/message formats
stay here.

This is the second **date window** worked example — a window that fires once
and ages out, with no ``satisfied()`` beyond leaving the stage. The live
patents migration is a downstream follow-up.
"""

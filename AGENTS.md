...
Code earns a permanent home in `src/coga/` in **two ways**: (a) genuine **shared infra** — code with **≥2 real consumers** (compose, config, task/ticket IO, the launch machinery, shared parsers and gates); and (b) a reviewed command contract that must be co-versioned with the package. The second class contains the deliberately fixed `coga run` recipe registry (`open-pr`, `delete-task`, and the recurring jobs) plus a command only when its contract names the package-private invariant or atomic transaction that an edge implementation using stable CLI/filesystem interfaces could not preserve. Python logic or inability to use an alias does not establish that home. Existing command heads with unresolved placement remain in the package pending review; [coga/extension-model](docs/contexts/coga/extension-model/SKILL.md#open-command-placements) owns their classification status and the deferred reviews.

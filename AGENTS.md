...
`src/coga/` holds **only two kinds of code**: (a) genuine **shared infra** — code with **≥2 real consumers** (compose, config, task/ticket IO, the launch machinery, shared parsers and gates); and (b) a real **command implementation** that genuinely needs Python logic and can't be expressed as an alias. The latter includes functions in the explicit `coga run` recipe registry (`open-pr`, `delete-task`, and the recurring jobs) as well as commands such as `coga digest` and `coga megalaunch`.

**Everything else stays at the edge.** A single-consumer helper may live beside the ticket or skill that uses it, import only shared core infra, and be invoked explicitly by agent instructions; core must never import from a ticket or skill directory. **"Backs a CLI spelling" is not by itself a pass into core**: a launch-target command is an argv rewrite in `[aliases]` (`dream = "recurring launch dream"`), not a Typer command with logic. Deterministic behavior that needs a stable headless command contract must be a fixed name in `runner.RECIPES`; that is a genuine package command, so its implementation remains importable under `src/coga/`. Skills describe how to invoke commands rather than supplying executable launch plugins. See the `coga/codebase` context for the full rule.

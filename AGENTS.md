...
`src/coga/` holds **only two kinds of code**: (a) genuine **shared infra** — code with **≥2 real consumers** (compose, config, task/ticket IO, the launch machinery, shared parsers and gates); and (b) a real **command implementation** that genuinely needs Python logic and can't be expressed as an alias. The latter includes functions in the explicit `coga run` recipe registry as well as commands such as `coga digest` and `coga megalaunch`.

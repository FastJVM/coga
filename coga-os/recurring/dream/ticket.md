...
separate REM task under `coga-os/recurring/`, with its own cadence, skill
...
- its resolved task directory under `coga-os/tasks/` still exists; and
...
`last_serviced_period` line in `coga-os/recurring/<name>/blackboard.md`
...
directory under `coga-os/tasks/`. The deletion goes in the PR, not the working

...
3. **External scripts / tools** — *stateless, parameterized* invocations. Two kinds:
   ...
   - **External script / service** — a *Coga-authored* stateless capability that
     ...
     `coga-os/scripts/` target `launch` can call) is still to be designed — the
...
- **What launch consumes / what precedes it** — the `create` primitive (the ticket
  factory whose output launch runs) and fresh `init` (creates the `coga-os/` a
...
`coga-os/` files) and dogfooding (Coga's own operations exercise the script-step
model):

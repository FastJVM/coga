...
- **`coga-os/`** — the user-facing OS layout (config, tasks,
...
generated `coga-os/.agent-skills/` view, which exposes the same effective
...
`coga-os/skills/` through the public skill installer during init/update; they
...
`src/coga/resources/templates/coga-os/bootstrap/{skills,contexts,workflows}/`,
not in the live `coga-os/bootstrap/` of a working repo — that copy is
...
- **Force-add every battery edit, not just new files.**
  `src/coga/resources/templates/coga-os/.gitignore` ignores `bootstrap/` (it
...
- **Skill Python deps via `requirements.txt`.** A skill declares its
  ...
  `coga-os/**/skills/**/requirements.txt` into `.coga/.venv` on `coga init`
  and `coga init --update`, after package-backed batteries are materialized
  into `coga-os/bootstrap/` and after managed optional skills have had a
  chance to install into `coga-os/skills/`. That ordering is what makes a
...
path: `coga/resources/templates/coga-os/skills/_template/SKILL.md`.
```

...
- Network or webhook-rejection error during `requests.post` →
  ...
  appended (tagged with that task's ref) to the repo-global `coga-os/log.md`.
...
- Live callers (`post`): `commands/panic.py`, `commands/slack.py`,
  ...
  in the repo-global `coga-os/log.md`, tagged with the task ref.
...
the resolved task directory path under `coga-os/tasks/` (top-level or nested
...
- Git operation failures (missing git, invalid repo state, commit failure,
  ...
  loud: stderr plus a repo-global `coga-os/log.md` line, then `typer.Exit(1)`. The same-branch push
...
only in the global `coga-os/log.md` and git? Don't add silent state mutations that bypass both

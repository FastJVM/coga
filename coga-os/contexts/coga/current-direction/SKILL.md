...
- **Recurring runs use a stable path-qualified task ref.** The current direction is
  `coga-os/tasks/recurring/<name>/` (`recurring/<name>` in CLI/status/Slack),
  ...
  the repo-global `coga-os/log.md` remains append-only human history, not the dedup source.
...
- **The bundled `eval/ticket-diagnostic` skill is kept.** Its removal was
  ...
  `src/coga/resources/templates/coga-os/bootstrap/skills/eval/ticket-diagnostic/`
  stays on main (no live `coga-os/skills/eval/` override exists). Do not
...
- **Dream is a recurring task template plus an alias.** The standalone
  ...
  `coga-os/recurring/dream/` — an ordinary recurring template. `coga

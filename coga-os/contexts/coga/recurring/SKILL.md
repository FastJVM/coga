---
name: coga/recurring
description: How Coga's recurring task system works — recurring tasks as ticket-format directories under coga-os/recurring/, the creation contract, period-task naming, and where last-run state persists. Attach to any ticket that adds or changes a recurring task.
---
...
A recurring task lives under `coga-os/recurring/<name>/` and has the same
...
line to the repo-global `coga-os/log.md` (tagged `recurring/<name>`) each time
...
- `coga recurring` (bare) — scans every recurring task, get-or-creates the
  stable instantiated task at `coga-os/tasks/recurring/<name>/`, records the
...
`coga-os/tasks/recurring/<name>/`, with its own fresh blackboard. That task
...
blackboard region: the part of `coga-os/recurring/<name>/ticket.md` below the
...
- **Instantiated task ref** is `recurring/<name>`, backed by
  `coga-os/tasks/recurring/<name>/`. The `recurring/` directory is the
...
- **`last_serviced_period` in the template blackboard is the period
  ...
  `coga-os/recurring/<name>/ticket.md` before creating: if
...
- **Run history goes to the repo-global `coga-os/log.md`** (tagged
...
- Do not store last-run state in the instantiated task's blackboard under
  `coga-os/tasks/recurring/<name>/` — it is fresh for one run and deleted on
  cleanup. Use the recurring task's own blackboard region in
  `coga-os/recurring/<name>/ticket.md`.

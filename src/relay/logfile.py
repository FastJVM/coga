...
Relay keeps one audit log per repo at `relay-os/log.md` (not one per task). Each
...
ignore the log entirely. `relay-os/.gitattributes` marks `log.md` `merge=union`
...
from relay.paths import log_path, recurring_dir, tasks_dir
...
def append_log(cfg: Config, task_ref: str, actor: str, message: str) -> None:
    """Append one line to the repo-global `relay-os/log.md`, tagged `task_ref`."""
    path = log_path(cfg)
...
def last_activity_map(cfg: Config) -> dict[str, datetime]:
    ...
    skipped. `relay status` uses this instead of re-scanning per task.
    """
    path = log_path(cfg)
...
def task_log_lines(cfg: Config, task_ref: str) -> list[str]:
    ...
    Filters the global log on the `[<task-ref>]` tag. Used by `relay show` to

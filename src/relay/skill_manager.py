"""Relay-managed skill install, update, status, and removal helpers."""
...
def install_url_skill(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="relay-skill-url-") as tmp:
...
def render_update_pr_body(summary: SkillUpdateSummary) -> str:
    lines = [
        "## Summary",
        "",
        "- Ran `relay skill update --all` for Relay-managed skills.",
    ]
    ...
    *,
    ...
    if not isinstance(url, str) or not url:
        return SkillResult(
            ...
            source_type="url",
            status="failed",
            message="source_url missing from Relay skill metadata",
        )
    ...
    if selector is not None and not isinstance(selector, str):
        return SkillResult(
            ...
            source_type="url",
            status="failed",
            message="selector in Relay skill metadata is not a string",
        )

    try:
        data = (downloader or download_url)(url)
        with tempfile.TemporaryDirectory(prefix="relay-skill-update-") as tmp:
            materialized = materialize_url_skill(url, data, Path(tmp), selector)
...
def _status_url_skill(
    ...
    *,
    ...
    try:
        url = metadata.get("source_url")
        if not isinstance(url, str) or not url:
            raise SkillManagerError("source_url missing from Relay skill metadata")
        selector = metadata.get("selector")
        if selector is not None and not isinstance(selector, str):
            raise SkillManagerError("selector in Relay skill metadata is not a string")
        data = (downloader or download_url)(url)
        with tempfile.TemporaryDirectory(prefix="relay-skill-status-") as tmp:
            materialized = materialize_url_skill(url, data, Path(tmp), selector)
        upstream_changed = (
...
def _replace_skill_tree(source: Path, target: Path) -> None:
    backup = target.parent / f".{target.name}.relay-update-backup"
    if backup.exists():

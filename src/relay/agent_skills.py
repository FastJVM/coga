...
def refresh_agent_skill_view(relay_os: Path) -> AgentSkillViewResult:
    """Rebuild `relay-os/.agent-skills` from local + bundled skills.

    The generated view is what Claude Code and Codex scan. Local skills under
    `relay-os/skills` win over bundled package-backed batteries under
    `relay-os/bootstrap/skills`.
    """
    view_dir = relay_os / AGENT_SKILLS_DIRNAME
    _remove_existing(view_dir)
    view_dir.mkdir(parents=True, exist_ok=True)

    refs: dict[str, Path] = {}
    # Bootstrap first, local second: local overrides exact bundled refs.
    refs.update(_skill_refs(relay_os / "bootstrap" / "skills"))
    refs.update(_skill_refs(relay_os / "skills"))

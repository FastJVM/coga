...
1. YAML frontmatter (`--- ... ---`), parsed by `relay.ticket.Ticket`.
...
log is **not** a region here: it lives in one repo-global `relay-os/log.md`
(see `relay.logfile`). That is the whole point of the single-file format — the
...
- **Frontmatter / step writers** (`relay bump`, `relay mark`, …) go through
  `relay.ticket.Ticket`, which re-renders the YAML and treats the whole body
...
def join_task_body(body_above: str, blackboard_text: str) -> str:
    ...
    Used by scaffolding (`relay create`) to write one `ticket.md` from a body

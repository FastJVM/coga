...
def relay_os_root() -> Path:
    ...
    for candidate in [cwd, *cwd.parents]:
        relay_os = candidate / "relay-os"
        if (relay_os / "relay.toml").is_file():
            return relay_os
        if candidate.name == "relay-os" and (candidate / "relay.toml").is_file():
            return candidate
    raise RuntimeError("could not find relay-os root")


def repo_root(relay_os: Path) -> Path:
    env_root = os.environ.get("RELAY_REPO_ROOT")
    if env_root:
        return Path(env_root)
    return relay_os.parent if relay_os.name == "relay-os" else relay_os
...
def pr_touches_task(paths: set[str], slug: str) -> bool:
    prefix = f"relay-os/tasks/{slug}/"
    exact = f"relay-os/tasks/{slug}"
...
def main() -> int:
    try:
        relay_os = relay_os_root()
        candidates = find_candidates(relay_os)
        delete_skill = delete_skill_path(relay_os)
        pr_state = None
        if candidates and delete_skill.is_file():
            pr_state = open_pr_state(repo_root(relay_os))

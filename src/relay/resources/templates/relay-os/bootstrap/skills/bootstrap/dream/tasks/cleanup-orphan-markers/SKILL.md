...
contains the processed Retro marker but whose task directory still exists.
Normal Retro PRs delete the source task directory in the same PR, so this skill
should usually report no work.
...
- Idempotency: a source task is skipped when its directory is already gone, the
  marker is absent, the task is not exactly `status: done`, or an open PR is
  already touching `relay-os/tasks/<slug>/`.

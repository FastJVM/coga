...
description: How to bring an external Agent Skill into a Relay repo — when to import, adapt, or write from scratch, how to find a candidate, and how provenance is recorded so a human can see what was imported and what changed locally.
---

# Import an external skill

Relay skills are plain `SKILL.md` files — the same format Claude Code, Codex,
...
`relay skill` — the CLI (`install-url`, `status`, `update`) is the *mechanism*
...
3. **Write local** — nothing external fits, or the only candidates are broad
   ...
   focused skill under `relay-os/skills/<ns>/<name>/` per `bootstrap/ticket`
   step 4. No provenance file — it's original.

When import and write are close, prefer **import**: a skill with provenance is
one a future `relay skill update` can refresh against upstream; a hand-written
...
Provenance is **not** a markdown block you write by hand. `relay skill
install-url <url> [path-inside-archive]` installs the skill as a plain
directory and writes a `.relay-source.json` (`schema: relay.skill-source.v1`)
...
- `source_digest` / `source_tree_digest` / `installed_tree_digest` — content
  hashes that let `relay skill status` and `relay skill update` tell a clean
...
- `local_adaptation_notes` — **hand-edit this.** When you adapt (decision 2),
  ...
  future human reads to understand the fork; `relay skill update` preserves it
...
`relay-os/skills/<namespace>/<name>/`, resolved before the bundled batteries
like every other local skill. They stay plain directories — no package cache,
no hidden service owns them — so the `.relay-source.json` sitting beside the
...
4. **Decide** — import the clean one; reject the `jest` one (broad + wrong
   ...
   `python -m pytest` → **adapt**: `relay skill install-url <url>`, change the

...
The Gmail analogue of `relay/google-calendar`: a shared, bootstrapped capability
any relay skill can use to read a mailbox and pull attachments, without a
...
- `authorize` → prints the `[gmail]` block to paste into `relay.local.toml`
...
mailbox to read) and paste the printed `[gmail]` block into `relay.local.toml`.

## Dependencies

`google-api-python-client`, `google-auth`, `google-auth-oauthlib` (in
`requirements.txt`), installed into `.relay/.venv` by relay's per-skill install
pass at bootstrap / `relay init --update`.

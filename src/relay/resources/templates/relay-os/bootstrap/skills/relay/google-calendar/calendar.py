#!/usr/bin/env python3
"""Google Calendar capability — a self-contained relay skill script.

The reusable way for relay skills to create/read/update/delete Google Calendar
...
Auth — service account
  Reads a service-account JSON key whose path comes from `relay.local.toml`
...
skill's requirements.txt and installed into `.relay/.venv` at bootstrap by
relay's per-skill install pass.
"""
...
def resolve_credentials_path() -> str:
    """Read `[calendar].service_account_file` from relay.local.toml.

    Supports `env:VAR` indirection (same convention as relay's `[secrets]`).
    ...
    if not value:
        _bail(
            ...
            "[calendar].service_account_file in relay.local.toml (a path to the "

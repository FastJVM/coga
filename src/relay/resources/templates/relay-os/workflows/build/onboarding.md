...
This is the user's first run after `relay init`. `relay init` already captured
...
- Write the agreed vision to **`relay-os/contexts/product/vision/SKILL.md`** in
...
ticket is generated from, so a later `relay launch <slug>` is oriented by the
...
(surfaced as `relay create <slug>`); if/when `relay ticket` consolidates
creation (`marketing/relay-ticket-creates`), use `relay ticket <slug>` instead.
...
batch — launch any one with `relay launch <ticket-slug>`." Then `relay mark
done`.

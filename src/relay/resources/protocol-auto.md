...
- **Either proceed or panic.** When you hit uncertainty, there are two valid
  ...
  1. Proceed with the best reasonable choice. Document the choice and its
     rationale in the blackboard's Decisions section *before* you act on it.
...
- **Log decisions, not narration.** Findings go to the blackboard's Findings
  section. Decisions (with reasons) go to Decisions. Don't write "I'm about
  to run the tests now" — write what the tests said.
- **Be conservative about scope.** With no human to correct you, drift is the
  ...
  issues, write them to Notes — don't fix them.

...
Exposed as `coga validate` (see `coga.commands.validate`); also runnable
directly as a module:

    coga validate [--json] [--task <slug>] [--fix] [--max-blackboard-kb N] [--check-slack] [--check-github]
...
def _check_frontmatter_schema(
    ...
    for field_name, spec in cfg.ticket_fields.items():
        ...
        # constraints only apply to non-empty values; `coga mark active`
        ...
        if ticket.status in {"active", "in_progress", "blocked", "paused"}:
            out.append(Issue(
                ...
                message=(
                    ...
                    "be advanced (`coga bump` has no step to move). A workflow "
...
def _main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Coga repo validator")

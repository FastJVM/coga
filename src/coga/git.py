...
class FileMutationRollback:
    ...
    Call ``arm`` after the command has written its generated ticket/log state
    and immediately before strict publication. ``restore`` rewinds an ordinary
    path only while its bytes still equal that generated snapshot. Append-only
    union paths instead remove just the generated lines, retaining additions a
    concurrent command made before publication failed.
    """
    ...
    def arm(self) -> None:
        """Record the exact generated bytes publication is about to consume."""
        self.generated = {
            path: path.read_bytes() if path.is_file() else None
    ...
    def restore(self) -> tuple[Path, ...]:
        ...
        # Pre-publication validation can fail before a caller reaches its arm
        # callback. No remote operation ran in that case; snapshot the current
        # generated state now so the same conditional restore path applies.
    ...
    *,
    ...
    if require_single_push_url:
        ...
        if (
            ...
            and _working_tree_bytes(root, preserve_union_rel) is None
        ):
            return _FeaturePublicationState(
                aligned=False,
                may_commit=False,
...
def feature_publication_lease(
    ...
    try:
        return _feature_publication_lease(cfg, task_path, branch)

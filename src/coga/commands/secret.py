"""`coga secret get <ref>` — resolve and print one secret reference.
...
`coga launch` uses, so the two cannot diverge: same `op` shell-out, same
...
def get(
    ...
    Resolves through the same `select_launch_secrets` helper `coga launch`

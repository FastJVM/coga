...
app = typer.Typer(
    help="Install, update, remove, and inspect Coga-managed skills.",
...
def install_url(
    ...
    force: bool = typer.Option(
        False,
        "--force",
        help="Overwrite a locally adapted URL skill and reset Coga provenance notes.",
    ),
) -> None:
    """Download a non-GitHub URL, install locally, and preserve Coga metadata."""
    cfg = _load_config_or_exit()
    try:
...
def update(
    ...
    all_skills: bool = typer.Option(
        False,
        "--all",
        help="Update all Coga-managed skills.",
    ),
    ...
    try:
        ...
        if pr:
            if not all_skills:
                raise SkillManagerError("--pr is only supported with `coga skill update --all`.")
            commands = verify or ["coga validate --json"]

...
class DuplicateTaskSlugError(Exception):
    ...
    than silently dropping a task. `coga validate` catches this and reports
...
class TaskRef:
    ...
    def id_slug(self) -> str:
        ...
        in `coga status`, Slack, and the launch arg. The `.md` suffix of a
...
def is_under(directory: str | None, target: str) -> bool:
    ...
    The subtree test shared by `filter_tasks_under` (the `coga status <dir>`
    filter) and the `tasks/recurring/` split in `coga status` — `target` and
...
def filter_tasks_under(
    ...
    read-only contract of `coga status` holds.
    """
    if directory is None:
        if recurse:
            return refs
...
def resolve_task(cfg: Config, task_arg: str) -> TaskRef:
    ...
    nested task by its path (`marketing/coga-crm`). Accepts an exact match or

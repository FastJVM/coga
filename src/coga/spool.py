...
Producers `append_record` as events happen; the single consumer (`coga digest`)
...
consistent with Coga's no-hidden-state rule.
...
coga processes (this clone, another clone, another machine), so the spool is a
...
content). See the `coga/sync` context for the full contract.

...
def _require_supported_python(version_info: tuple[int, ...] = sys.version_info) -> None:
    ...
    Importing any `coga.*` submodule imports this package first, so this guard
    ...
    if tuple(version_info[:2]) < (3, 11):
        ...
        raise RuntimeError(
            "Coga requires Python 3.11 or newer (it uses the stdlib `tomllib`); "
            f"this interpreter is Python {got}. Run Coga with a 3.11+ interpreter "
            "— e.g. `python3.12 -m coga.cli` — or install it into a 3.11+ "
            "virtualenv."
        )

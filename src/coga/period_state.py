...
in its `ticket.md` frontmatter. When `coga recurring` creates a period task,
...
period task completes (`coga mark done`), we diff: a declared key still equal
...
surface (the local warning echo and the Slack FYI) lives in `coga.mark`, and
the static sweep lives in `coga.validate`; both reuse the helpers here.
"""
...
# (ticket/blackboard/log) without being mistaken for one, and `coga validate`'s

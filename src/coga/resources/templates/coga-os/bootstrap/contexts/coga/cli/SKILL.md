...
Scaffold `coga-os/` in `PATH` (default `.`), or with `--update` refresh
the coga-managed bits in the current repo.

- `coga init mycompany` — fresh scaffold; refuses if `coga-os/` exists.
...
- `coga init --update --all [PATH]` — sweep mode. Scan `PATH` (default the
  ...
  `coga-os/coga.toml` — and run the `--update` refresh in each. One
...
`pip install coga-os` installs bundled batteries into the wheel as package
resources. It does not modify a repo. `coga init` and `coga init --update`
materialize those package resources into `coga-os/bootstrap/`, where Coga
resolves them after project-local `coga-os/skills` and `coga-os/contexts`.
...
`coga-os` package is left installed and the command prints the exact pipx/pip
...
`--workflow <name>` (path under `coga-os/workflows/`) is optional *in
...
coga-os/tasks/<dir>` to make one (`mkdir -p` to nest), `mv` a task directory
...
history from the repo-global `coga-os/log.md` to the
...
Manage project-local skills under `coga-os/skills/`. `coga skill install`
and `coga skill install-*` never write into `coga-os/bootstrap/`; bootstrap
...
path: upgrade `coga-os`, then run `coga init --update`.
...
`gh skill ... --dir coga-os/skills`. Constraints that come with that
substrate:
...
- `gh skill update --dir` has a known bug that relocates or deletes skills in
  ...
  `coga-os/skills/<ns>/<name>/` layout so `--dir` updates stay safe.
...
`coga-os/recurring/dream/` recurring task and launches it interactively.
...
the blackboard region of `coga-os/recurring/dream/ticket.md` as
...
Scan `coga-os/recurring/`, then scaffold and launch every task that is due.
...
get-or-create the current run at `coga-os/tasks/recurring/<name>/` and advance
...
nothing runs unless you invoke it. `coga-os/scripts/cron.sh` is the
optional entry point if you later wire it into a scheduler yourself.
Dedup after Dream deletes a completed run comes from
`last_serviced_period >= current period_key`; the repo-global `coga-os/log.md`
...
schedule. `name` is the directory name under `coga-os/recurring/`. The task

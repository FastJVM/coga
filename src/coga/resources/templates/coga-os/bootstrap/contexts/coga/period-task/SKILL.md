...
task directory under `coga-os/tasks/recurring/<name>/` is the scratch space
...
`coga-os/recurring/<parent-name>/`. Its blackboard region (in `ticket.md`,
...
`coga-os/recurring/<parent-name>/ticket.md`.

Every period-task run that carries state follows the same shape:

1. At the start, read the blackboard region of
   `coga-os/recurring/<parent-name>/ticket.md` to find the current `last_serviced_period` and where the previous run
   stopped.
...
Your own blackboard (the region of `coga-os/tasks/recurring/<parent-name>/ticket.md`) is fresh for

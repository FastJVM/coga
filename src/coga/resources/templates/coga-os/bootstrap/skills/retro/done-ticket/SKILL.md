...
- Inputs: each source task's `ticket.md` (body + blackboard region) and its history in the repo-global `coga-os/log.md`, plus
  every local and bundled context/skill file under `coga-os/contexts/`,
  `coga-os/bootstrap/contexts/`, `coga-os/skills/`, and
  `coga-os/bootstrap/skills/`, loaded once per run before ticket-by-ticket
  extraction.
- May change: warranted context files, warranted skill files, and the exact
  resolved source task directories under `coga-os/tasks/` for every processed
...
- read every context file under `coga-os/contexts/**/SKILL.md` and
  `coga-os/bootstrap/contexts/**/SKILL.md`;
- read every skill file under `coga-os/skills/**/SKILL.md` and
  `coga-os/bootstrap/skills/**/SKILL.md`;
...
state** of the local roots (`coga-os/contexts/**`, `coga-os/skills/**`) plus
the bundled roots (`coga-os/bootstrap/contexts/**`,
`coga-os/bootstrap/skills/**`). Load that corpus once at the start of the run.
...
live either at `coga-os/tasks/<slug>/` or one level deeper in a group directory.
...
- `coga-os/contexts/**/SKILL.md`
- `coga-os/bootstrap/contexts/**/SKILL.md`
- `coga-os/skills/**/SKILL.md`
- `coga-os/bootstrap/skills/**/SKILL.md`
...
1. **Inventory contexts once.**
   Read all `coga-os/contexts/**/SKILL.md` and
   `coga-os/bootstrap/contexts/**/SKILL.md`. For each context, note its path,
...
2. **Inventory skills once.**
   Read all `coga-os/skills/**/SKILL.md` and
   `coga-os/bootstrap/skills/**/SKILL.md`. For each skill, note its path,
...
3. **Read ticket evidence one ticket at a time.**
   For each selected slug, read its `ticket.md` (body + blackboard region) and its lines in the repo-global `coga-os/log.md`.
...
6. **Classify each candidate.**
   ...
   | New coherent topic | Create a focused context under `coga-os/contexts/<namespace>/<name>/SKILL.md`. |

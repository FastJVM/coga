...
The calendar analogue of `coga slack`: a shared, bootstrapped capability any
coga skill can use to write Google Calendar events, without a per-machine
...
```
python <coga-os>/skills/coga/google-calendar/gcal.py get \
...
`env:VAR` indirection is supported (same convention as coga's `[secrets]`).
...
Coga's bootstrap installs every `coga-os/skills/**/requirements.txt` into
`.coga/.venv` (during `coga init` / `coga init --update`), so a bootstrapped
skill brings its own deps. If they're missing, run `coga init --update`.

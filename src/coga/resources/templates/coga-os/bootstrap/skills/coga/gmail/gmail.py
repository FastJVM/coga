#!/usr/bin/env python3
"""Gmail capability — a self-contained coga skill script.

The reusable way for coga skills to read a Gmail mailbox — search, fetch a
...
or `coga slack`):
    ...
    python <coga-os>/skills/coga/gmail/gmail.py search \
...
`[gmail]` block to paste into `coga.local.toml`. Exit codes are the contract:
...
def _coga_os_root() -> Path:
    """Locate the coga-os dir holding coga.local.toml.
...
def resolve_gmail_config() -> dict[str, str]:
    """Read and validate the `[gmail]` block from coga.local.toml.

    Each value supports `env:VAR` indirection (same convention as coga's
    ...
    if not isinstance(section, dict):
        _bail(
            ...
            "coga.local.toml (run `gmail.py authorize` to mint one)."
        )
...
def op_authorize(client_secret_file: Path) -> dict[str, Any]:
    ...
    sys.stderr.write(
        ...
        "coga-os/coga.local.toml (it is gitignored):\n"
    )

...
The canonical ticket shape is `coga-os/tasks/_template/ticket.md`. **Read it
...
- `coga-os/tasks/<slug>/ticket.md` — any existing code-change ticket with
...
- `ls coga-os/workflows/ coga-os/bootstrap/workflows/` (and one level
  deeper) — known workflows. Repo-local workflows live under
  `coga-os/workflows/`; bundled batteries (e.g. `code/with-review`) under
  `coga-os/bootstrap/workflows/`. A local file overrides a bundled one
  with the same ref.
- `ls coga-os/contexts/*/` — known contexts (path shape:
  `coga-os/contexts/<namespace>/<name>/SKILL.md`; reference shape in
  tickets: `<namespace>/<name>`).
- `ls coga-os/skills/*/` — known skills (same path/reference shape).
...
3. **Autonomy triage** — before settling the workflow, run the three-question
   test from `coga-os/contexts/autonomy/triage/SKILL.md` against the task as
...
4. **Workflow** — which workflow fits? `ls coga-os/workflows/
   coga-os/bootstrap/workflows/` for the options (e.g. `code/with-review`
...
- New context: `coga-os/contexts/<namespace>/<name>/SKILL.md` with
  ...
  process.
- New skill: `coga-os/skills/<namespace>/<name>/SKILL.md` with the same
...
Read `coga-os/workflows/<name>.md` (or
`coga-os/bootstrap/workflows/<name>.md` for a bundled battery) for the

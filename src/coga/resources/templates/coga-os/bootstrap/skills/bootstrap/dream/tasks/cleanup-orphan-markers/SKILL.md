...
- Inputs: task directories under `coga-os/tasks/`, the source task's
...
- `coga-os/tasks/<slug>/ticket.md` has exact `status: done`;
- the `ticket.md` blackboard region of `coga-os/tasks/<slug>/` has a `## Retro` block containing both
...
- no open PR is already touching `coga-os/tasks/<slug>/`;

...
def coga_os_root() -> Path:
    ...
    for candidate in [cwd, *cwd.parents]:
        coga_os = candidate / "coga-os"
        if (coga_os / "coga.toml").is_file():
            return coga_os
        if candidate.name == "coga-os" and (candidate / "coga.toml").is_file():
            return candidate
    raise RuntimeError("could not find coga-os root")
...
def repo_root(coga_os: Path) -> Path:
    ...
    if env_root:
        return Path(env_root)
    return coga_os.parent if coga_os.name == "coga-os" else coga_os
...
def delete_skill_path(coga_os: Path) -> Path:
    ...
    A repo may override the skill under `coga-os/skills/`; otherwise it is
    materialized into `coga-os/bootstrap/skills/` by `coga init`. Mirrors
...
def pr_touches_task(paths: set[str], slug: str) -> bool:
    prefix = f"coga-os/tasks/{slug}/"
    exact = f"coga-os/tasks/{slug}"

...
This is the deterministic recipe for the `coga/autoclose/sweep` skill (run by
the `autoclose-merged` recurring sweep). It is a single-consumer maintenance
recipe, so under the microkernel policy it lives in the skill dir rather than in
core `src/coga/`. The `## Dev` PR-link *parsers* and the `gh`-facing helpers
(`GhError`, `pr_state`, `parse_pr_url`) have other consumers — `branchcleanup`
(used by `coga retire`), the branch-sweep recipe, the Dream orphan-marker
worker — so they are genuine shared infra and stay in `coga.autoclose`; this
recipe imports them from there.
...
from coga.autoclose import GhError, parse_pr_url, pr_state
...
# Re-exported from `coga.autoclose` so the skill's `run.py` can pull the whole
# sweep surface (`sweep_merged`, `GhError`) from this one recipe module.
__all__ = [
    "GhError",
    "sweep_merged",
    "parse_pr_number",

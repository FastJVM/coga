...
`requirements.txt`). Coga installs nothing for a skill — run
`python -m pip install -r requirements.txt` from this skill's dir, using the
same Python that runs `gmail.py`.

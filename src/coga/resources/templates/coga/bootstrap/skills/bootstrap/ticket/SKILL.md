...
- `coga/tasks/<slug>.md` or `coga/tasks/<slug>/ticket.md` — any existing
  code-change ticket with `contexts:` + `workflow:` filled in works as a model
  to mimic. A self-contained ticket uses the flat form; one with a sibling
  script or attachment uses directory form.
...
4. If editing an existing non-draft ticket
   ...
   sections you used.
5. Re-read the ticket and verify current requirements live under `## Description`
   or `## Context`, superseded designs remain in their blackboard section,
   and the blackboard cleanup matches the ticket status.

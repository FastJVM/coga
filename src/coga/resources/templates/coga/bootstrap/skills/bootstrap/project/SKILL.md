...
The human is at the keyboard in an attended agent launch. Ask, don't guess. Keep the
...
- Leave `workflow: null` unless the human chose a workflow for that step — a
  ...
  each ticket when they're ready. Don't run `coga mark active` or
  `coga launch` on anything.

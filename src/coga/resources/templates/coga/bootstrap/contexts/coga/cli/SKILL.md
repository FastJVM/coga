...
3. **Launch** — each cleared ticket is re-read and reclassified when its own
   ...
   pending control ticket. Another megalaunch refuses either claim form, and
   ...
   post-release admission failure kills the child but keeps its launch audit.
   It never compensates backward to `active`. Step advancement and lifecycle
   transitions that end or park an admitted session clear the plain generation;
   ordinary `coga launch` is the explicit recovery path for that admitted form.

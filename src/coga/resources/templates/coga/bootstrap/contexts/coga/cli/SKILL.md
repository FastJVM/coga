...
`--workflow <name>` (resolved from `coga/workflows/`, then the package's
`bootstrap/workflows/`) is optional *in draft only*. A workflow-less draft is
a valid authoring state; the workflow can be added to the ticket any time
before activation. The bumpability gate lives at activation, not here:
`coga mark active` refuses a workflow-less ticket with an error pointing at
`--workflow` or `coga ticket`, and `coga validate` reports a workflow-less
`active`/`in_progress`/`paused` ticket as an `active-no-workflow` **error** (a
stuck task no `coga bump` can advance). Once a ticket leaves `draft`, a
workflow is mandatory. For guided authoring that fills the workflow in for
you, use `coga ticket`.
...
hold the checkout-local state lock for the filesystem removal, so
...
3. **Launch** — each cleared ticket is re-read and reclassified when its own
   ...
   preflighted `in_progress` bytes. An unclaimed Git-backed megalaunch start or
   resume publishes a unique, visible `pending:<uuid>` launch generation while
   its spawned child is held; every Coga task publisher refuses to replace that
   pending control ticket. Another megalaunch refuses every claim form, and
   ordinary `coga launch` also refuses pending admission. The claim is one
   `git.publish`; its provenance check is the compare-and-swap, and a refused
   or failed claim restores the pre-write ticket and retracts its audit lines
   while an uncertain one keeps the pending state as reconciliation evidence.
   The shared `validate_before_spawn` seam rereads those local bytes and
   freshly verifies the whole ticket on control before the launch audit;
   `validate_after_spawn` repeats that proof after the PTY child exists but
   while a private pipe still holds it before exec. The parent then appends
   the audit and repeats that held-child proof after the append. Only the
   second success writes the gate byte. Before dropping the same local state
   lock, a post-release callback removes only `pending:` and publishes the
   same UUID with `expect` pinned to the pending control copy. A lifecycle
   change arriving from another checkout after the
   final fetch is therefore refused while pending or observes the admitted
   plain UUID only after the child can execute. A changed or unverifiable claim
   removes the owned audit append, refuses the child, and retains pending state
   for reconciliation; an audit failure also kills the held child. A
   post-release admission failure kills the child, keeps its launch audit, and
   retains a local `released:<uuid>` witness. It never compensates backward to
   `active`. An explicit ordinary launch reconciles that witness only when the
   whole control ticket is the matching pending or already-admitted revision,
   publishes the plain UUID, and refuses to spawn on any mismatch or Git
   failure. Step advancement and lifecycle transitions that end or park an
   admitted session clear the plain generation; ordinary `coga launch` is also
   the explicit recovery path for that admitted form.
   Git-disabled megalaunch uses a plain UUID behind the local state lock.
   Preflight also materializes the
...
fetch and fast-forward must succeed before period state is read or written.
...
system temp dir. A missing local control ref is seeded from the freshly
fetched remote-tracking ref, so single-branch and narrow-refspec clones never
rely on shared `FETCH_HEAD`. The child runs its scan from
...
refusal (an ahead or diverged control checkout) is reported once, naming the
exact `git pull --rebase` fix — no follow-up refresh/sync attempts re-failing
against the same divergence, and no new local commits deepening it. Repos serviced from a temporary control
...
REPL exit leaves the period unfinished. The runner's push preflight and
period verification/publication rules are in `coga/launch-internals`. Templates intended for cron or other unattended

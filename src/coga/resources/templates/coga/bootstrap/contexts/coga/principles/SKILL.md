...
**Receipt:** the CLI + files are the whole surface, no webUI; the two modes — `llm` for agent judgment and `script` for deterministic Python — route work to the right substance; per-step `assignee` (`agent`/`other-agent`/`human`)

...
- `coga recurring` (bare) — the public command head translates
  ...
  **configured agent does not hand off there** — `run_script_chain` sets
  ...
  Whichever of the three stops it, the sweep never again abandons work
  silently: `recurring_runner._record_abandoned_due` lists every due task
  admitted behind the stopping one under `## Unresolved recurring failures`
  in the run record as `admitted as due but never launched`, counts each in
  `problems:`, and adds one sweep note naming them. A report whose
  `tasks run:` is short of its due count therefore always says why.

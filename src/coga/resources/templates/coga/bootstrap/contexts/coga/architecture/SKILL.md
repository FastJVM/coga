...
refused because Git cannot update all of its destinations atomically.
...
state from the aligned tree before classifying its assignee. Strict alignment
also rejects unexpected staged, tracked, or untracked checkout dirt even when
the local tip already equals the remote; the pending union-safe audit log is
the sole explicit exception. Draft, paused, and blocked activation and
...
when those edits overlap. The checkout fast-forwards to that compensation when
it can do so without overwriting concurrent local state; the error still
escapes and the caller restores an ordinary file only while it still equals
the generated bytes, while removing generated audit lines union-safely from a
concurrently appended log. No child ran, no false lifecycle state remains
published, and no unrelated concurrent work was swept. Reproducible
...
append. The child inherits a task-scoped recorded-branch-and-PR capability,
...
without granting it to nested ordinary launches. If that resumed session exits

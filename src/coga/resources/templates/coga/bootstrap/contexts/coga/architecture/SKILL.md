...
TTY is refused before recorded-checkout or PR validation. Publication requires launch
...
bytes belong to Coga. Strict state writers therefore arm rollback immediately
after each generated file write, before post-write validation can reject it,
then re-arm after the audit append. Before replacing a ticket from an in-memory
state object, they also require its live bytes to equal the latest captured
revision. Their first blocker append or resolution is likewise conditional on
the captured full-ticket bytes before the blackboard splice, so it cannot
adopt a peer revision and label it generated. A peer edit between snapshot and
write is retained. A failure

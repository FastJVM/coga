...
**Where Coga is outclassed (state it plainly).** Fully-managed unattended autonomy out of the box → **Devin / Symphony / Claude Code** (Coga keeps the local, file-backed loop and uses megalaunch + recipes for unattended drain). Heavy parallel/dynamic orchestration → **frameworks** (Coga's workflow is a linear state machine). Managed reliability / support / SLA → **paid products** (Coga is self-hosted, self-supported). Zero-setup → **CLAUDE.md** (one file, nothing to learn). Distribution / brand → anyone funded. And none of the axes Coga wins are a *moat* — the whole combination is copyable; the only durable layer is direction (see the conclusion).
...
recipe-backed and agent-backed recurring work, checked notifications, atomic writes,
...
**The one-line read of the 20%:** the trust substrate exists — unattended
recurring work, liveness watchdogs, atomic writes, and checked notifications
have shipped. What's missing is **earned trust**: sustained evidence that those
mechanisms stay legible and recoverable, plus calibration that lets operators
stop watching without losing confidence. Complete Buckets A and B and Coga
crosses from "an excellent way to work with agents" to "the substrate that
runs the company" — the thesis, delivered.
...
2. **Prove the felt layer so admirers become fans.** A fan base forms around a
   *felt* experience, not an essay. The trust substrate has landed; repeated
   real runs now have to prove that recurring work, liveness limits, streaming,
   and checked notifications produce calm control instead of operator doubt.
   The path from read-taste to felt-taste runs through that operational proof
   and calibration backlog.

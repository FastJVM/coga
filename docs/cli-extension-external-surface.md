...
- A command implementation proven to need Python logic rather than a fixed
  alias rewrite. This is a shape test, not a grandfather clause for every
  command currently implemented in Python.
...
not ratified examples of irreducibility; the command-cleanup design ticket must
still prove whether their behavior belongs in core or can move.
...
> run? If yes, it is kernel. Otherwise, is it a fixed registered recipe or real
> Python command logic that cannot be expressed as an alias? If yes, it is one
> of the two reviewed exceptions. If a human or scheduler merely calls it to
> start or inspect work, it is movable.
...
- **Not bootstrap/kernel:** `launch` does not call it mid-flight, a fresh
  ...
  recipe, and it does not require irreducible Python command logic.
...
| Mid-flight launch dependency, fresh bootstrap, fixed recipe, or irreducible command implementation | Kernel | compose, state writes, secret inject, fresh `init`, registered recipes |

...
**Home:** `docs/`, paired with `relay-os/contexts/relay/extension-model/`. The
...
Relay's kernel remains `launch` and its dependency closure:
...
- Fresh `init`, because no launch can run before `relay-os/` exists.
...
For Relay-authored stateless capabilities outside the kernel and ticket model,
...
general plugin registry and not a `relay-os/scripts/` dispatcher.
...
grow because a command is important, convenient, or shipped by Relay today.
...
1. Resolve the skill path exactly as Relay does today, preserving local-over-
   bundled precedence.
...
tree is the exact local content Relay is about to trust. Upstream freshness is
...
no longer match the provenance Relay recorded.

GitHub-backed installs need a normalized Relay provenance record before compose
can verify them. The current URL-backed path writes `.relay-source.json`; the
GitHub-backed path delegates to `gh skill`. The extraction follow-up should
either write the same Relay sidecar after every external install or translate a
stable `gh skill` metadata format into Relay's schema. Compose must not fetch
from GitHub or depend on preview-only metadata behavior.

## External-script boundary

An external script/service is a Relay-authored one-shot outside both the kernel
and the ticket lifecycle.

It belongs here when all of these are true:

- **Stateless in Relay terms:** it may mutate files, call tools, and print
...
- **Relay-authored:** Relay owns the wrapper logic, even if it shells out to
  `gh`, `git`, `op`, or another operator tool.
- **Not bootstrap/kernel:** `launch` does not call it mid-flight, and a fresh
  `relay-os/` does not need it to exist before launch can run.
...
| Relay-authored stateless one-shot | External script | Skill acquirer wrapper |
...
Shape: ship the Relay-authored acquirer as an installable GitHub CLI extension,
for example a future `gh relay-skill ...` command. The extension owns its argv,
shells out to `gh skill`, writes Relay provenance, and leaves normal git-visible
file changes in `relay-os/skills/`.

Strengths:

- Reuses an operator dependency Relay already documents.
- Keeps GitHub-specific skill acquisition next to the GitHub CLI it wraps.
- Lets argument parsing stay in a real command surface, not in `relay.toml` or
...
Shape: move stateless Relay helpers into another Python package, console script,
or hosted endpoint that Relay calls or asks the operator to install.
...
- A separate local package is another install/update path for every Relay repo.
- A hosted service conflicts with Relay's local-first stance unless a real
  requirement demands the crossing.
- If the package imports too much of `relay`, the extraction is mostly cosmetic;
  if it duplicates too much, the schema and path logic drift.

### `relay-os/scripts/` dispatch target

Shape: put scripts in the repo and add a Relay dispatcher, or let `launch` call
...
- It gives Relay-authored shared tools no clean provenance or upgrade story
  across repos.
...
future non-GitHub helpers, and repo-local `relay-os/scripts/` remains just a
directory the operator can run directly, not a Relay command system.

The Relay CLI may keep a temporary compatibility shim, but only as raw argv
forwarding with a fail-loud installation hint. The canonical command should live
in the external CLI. If preserving `relay skill ...` requires reimplementing the
...
- Relay must not add transient `relay launch` parameters for external scripts.
- Relay must not grow a `[commands]` or `[scripts]` DSL in `relay.toml`.
...
- External scripts communicate by stdout, stderr, exit code, and git-visible file
  changes.
- External scripts may load Relay config for paths and shared settings, but they
...
- **Acquire outside.** `gh skill`, URL downloads, local imports, `op`, and `env`
  lookup are acquisition mechanisms. Relay may author wrappers around them, but
...
digest. Acquisition can be updated, replaced, or moved out of `src/relay/`
...
2. **Normalize provenance for all externally acquired skills.** Keep the
   `relay.skill-source.v1` shape or introduce a v2 deliberately. The acquirer
   must record the installed digest needed by compose. If `gh skill` gains a
   stable metadata contract, adapt it into Relay's schema rather than making
...
3. **Extract the skill acquirer.** Move `install`, `install-local`,
   ...
   and PR-summary generation. Relay core may temporarily delegate raw argv.
4. **Leave read-view migration to Pass 2.** `skill status` is a read view in the
   ...
   provenance, but the Relay CLI's read-view cleanup should stay with
...
- **`gh skill` preview churn.** If the wrapped command changes, Relay's acquirer
...
- **Provenance split-brain.** GitHub-backed, URL-backed, local, bundled, and
  ...
  normalize externally acquired skills into one Relay sidecar and define what
  "no sidecar" means: repo-authored or package-backed content, not externally
  managed content.
- **Generic dispatcher creep.** `relay-os/scripts/` is tempting because it is

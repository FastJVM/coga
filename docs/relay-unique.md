# Where Relay Is Unique

> **Scope.** This doc isolates Relay's uniqueness. It is not a full product
> thesis; see [`vision.md`](vision.md) and [`market-thesis.md`](market-thesis.md)
> for the larger argument.

## The Short Version

Relay is unique in the combination, not in any single primitive.

Tickets, agents, skills, markdown, recurring jobs, and task state machines all
exist elsewhere. Relay's claim is the way those pieces are held together:
...
Relay's most important architectural choice is that durable state is not hidden
in an app database.
...
Relay's workflow primitive is not just a sequence of automated steps. It is an
...
The assignee can change at each step. That means Relay can encode not only what
...
Relay is not manual-first and not autonomy-first. It is judgment-first.

Relay has several autonomy levels:
...
- recurring templates: stable work that runs repeatedly as normal Relay tasks.
...
Relay treats context as the appreciating asset.
...
This gives Relay a specific memory stance:
...
it was doing; Relay's context library changes what future agents know.

## 5. Skills Are Bound To Work, Not Just Agents

Relay uses the standard `SKILL.md` shape, but the product semantics are
work-bound.

In agent mode, a workflow step's skill is composed into the launch prompt. In
script mode, a skill can carry an executable script. Relay injects task metadata
through env vars, so the script runs inside the same task model as agent work:
ticket, blackboard, log, repo root, Relay root, and skill directory.
...
That gives Relay a simple but powerful recovery path:
...
Relay's no-UI posture is not "we did not get to it yet." It is part of the
architecture.
...
truth should stay in files, because that is what keeps Relay inspectable,
...
Relay's first users are probably not generic business operators.
...
They already understand git, terminals, diffs, PRs, and repo conventions. Relay
turns those instincts into a company operating substrate.

## Positioning Line

Relay is for operators who want agents to run company work without moving the
company into an agent platform.

Or, more concretely:

> Relay gives your repo workflows, recurring operations, agent handoffs,

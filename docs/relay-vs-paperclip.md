...
> **Scope.** This is a focused comparison between Relay and Paperclip as of
> 2026-06-06, based on Relay's local product docs and Paperclip's public docs.
> Paperclip is moving quickly; re-check its README and docs before using this in
> external copy.

## The Short Version

Relay and Paperclip both live near "agent work operating systems," but they are
not trying to be the same thing.

**Paperclip manages agents as a workforce. Relay manages work as repo state.**

Paperclip asks: how do I coordinate an AI company with roles, goals, budgets,
heartbeats, approvals, and dashboards?

Relay asks: how do I make company work legible, repeatable, automatable, and
...
Relay is earlier. It is an internal operating substrate being dogfooded in this
...
heartbeats, "AI company." Relay currently owns a sharper substrate: files, git,
contexts, skills, blackboards, workflow handoffs, and human-reviewed knowledge.

## Philosophy

| Question | Paperclip | Relay |
|---|---|---|
...
company." Relay's strongest story is "run agent work without moving your company
into an agent platform."

Relay is not anti-autonomy. It is anti-opaque autonomy and anti-defaulting every
task to the same autonomy level. Relay's model is to pick the right autonomy
level per task or step: human-attended, agent-run, script-run, recurring, or
review-gated.

## Feature Comparison

| Area | Paperclip | Relay |
|---|---|---|
| Source of truth | Server/API/database; local setup can use embedded Postgres and local storage | Markdown files under `relay-os/` plus git history |
...
| Execution | Heartbeats wake agents on schedule, assignment, mention, or manual trigger | `relay launch`, `relay recurring`, `relay recurring launch`, and `mode: script` |
...
| Skills | Skills are discoverable runtime capabilities injected by adapters | Skills are process knowledge bound to workflow steps; script skills run with Relay task env vars |
| Recurring work | Routines and heartbeats create tracked issues and wake agents | Recurring templates create period-keyed Relay tasks with normal ticket/blackboard/log artifacts |
...
| Multi-company | First-class multi-company isolation | Repo-scoped by default; multi-repo operation through git/Relay conventions |
...
Relay work feels like operating a repo:
...
Paperclip tries to make autonomous delegation manageable. Relay tries to make
...
Relay binds process knowledge to work. A workflow step names a skill; `relay
launch` composes that skill into the prompt for that step. In `mode: script`,
Relay runs the skill's script directly and injects task metadata through env vars
such as `RELAY_TASK_DIR`, `RELAY_TASK_BLACKBOARD`, and `RELAY_SKILL_DIR`.

The practical distinction:

- Paperclip: "this agent has these capabilities."
- Relay: "this step of this task is executed under this process contract."

That is one of the deepest product differences.

## What Relay Should Learn From Paperclip

Relay should take Paperclip seriously as category pressure. Paperclip shows that
people want:
...
Relay can add those without copying Paperclip's architecture if every new surface
remains a view, adapter, or remote over repo state.

## What Relay Should Not Copy

Relay should not become a worse Paperclip. Avoid:
...
Relay's edge is that operators can read and change the exact files the agents
...
Relay is the more opinionated substrate:

> Run agent work from your repo: tickets, context, handoffs, recurring
> operations, and audit trail as markdown.

The right comparison is not "Relay lacks Paperclip's UI." The right comparison
is "Paperclip makes agent management a product; Relay makes company operations
repo-native."
...
- [`relay-os/contexts/relay/principles/SKILL.md`](../relay-os/contexts/relay/principles/SKILL.md)
- [`relay-os/contexts/relay/architecture/SKILL.md`](../relay-os/contexts/relay/architecture/SKILL.md)

...
**Notifications are the coordination hub.** Not as an add-on, but as the ambient awareness layer that makes running multiple parallel agents tractable. Slack is the first backend: every urgent or explicit event and every ticket outcome posts to a shared channel as it happens. At small team sizes the whole channel is read by everyone, so plain-text posts reach the right person without elaborate routing. The channel is a scrollable record of what needs attention across the company, across all projects, across all agents. We dip in when something needs us, dip out otherwise.

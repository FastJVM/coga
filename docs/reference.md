...
- `--pick` — choose interactively from an arrow-key list of every non-terminal
  ...
  success releases the executable. A changed or unverifiable claim removes the
  owned audit append, refuses the child, and retains `in_progress`; an audit
  failure also kills the held child, so neither stale nor unrecorded work
  starts. It never compensates backward to `active`,

...
that sync from reaching spawn. When that reread finds peer-edited ticket bytes,
megalaunch returns `active` only when the revision still carries the unique
`launch_generation` written by this refused attempt and that compensation is
durably published. A refused or failed compensation restores the local
`in_progress` evidence instead of claiming success; an ambiguous push retains
the generated bytes for reconciliation. Every start and `in_progress` resume
rotates the visible claim before spawn, so another launcher's same-lifecycle
session wins untouched just like a different lifecycle such as `done`; only
the provably unlaunched attempt is compensated. `coga launch`'s

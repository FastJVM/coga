...
control revision from both starting. In a detached checkout, strict publication
also seals the exact generated bytes in a scoped detached commit and overlays
only those leaves on control, preserving newer sibling attachments. A
successful claim is therefore clean before final validation or CLI teardown,
rather than available for a later broad sweep to replay. The shared state guard
treats the generation as system-owned and permits a same-generation blackboard
or session-ending edit only from a committed ticket baseline that still equals
the freshly fetched control ticket. Each accepted detached edit advances that
baseline with another exact-leaf commit, so a later unblock, bump, or terminal
transition is not stranded behind its own earlier publication. A published
generation is non-reclaimable by another megalaunch, so a
later launcher cannot supersede it after the owner's final point-in-time
control fetch. Workflow advancement and lifecycle transitions that end or park
the session clear the generation; ordinary `coga launch` is the explicit
recovery path for an abandoned claim.

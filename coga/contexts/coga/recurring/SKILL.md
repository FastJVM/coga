...
1. **The sweep builds a run record as it works** — per period task: how the
   ...
   `skip (lease changed)`, with a sweep note. Neither is a problem: a changed
   lease usually means another checkout landed the same period first and runs
   it, and a live ticket left behind is resumed by the next sweep. The lease
   check ignores line endings, so a `core.autocrlf=true` checkout does not
   trip it on the sync's own checkout. It is

...
- Dirty feature checkout naming `coga/tasks/<slug>...` or `coga/log.md` in
  the separate-checkout layout → a control-plane write stranded in the feature
  checkout (a `## Dev` or blackboard edit made before changing back to the
  primary checkout). Carry anything still missing into the primary checkout's
  ticket copy, then discard the feature-checkout copy
  (`git checkout -- <paths>`) and re-run. Do not commit it to the feature
  branch — that manufactures a `ticket.md` merge conflict on the PR — and do
  not stash it. See the `dev/code` context.

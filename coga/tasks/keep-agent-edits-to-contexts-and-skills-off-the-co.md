...
- `coga/contexts/coga/sync/SKILL.md` ("Human hand-edits" and the pathspec
  list, ~lines 950-975) documents the sweep's purpose and already records
  the `bootstrap/` root-layout gap as the same class of problem.
- `coga/contexts/coga/principles/SKILL.md` §4 is the principle and names
...
- `COGA_TASK_*` is cleared and re-derived at every launch boundary
  (`coga/architecture`, environment section), so "is a launched agent
  present" is a reliable signal inside a session.
- Attach `coga/sync` and `coga/launch-internals` for the implement step;
  they own the publication invariants a change here can break.

...
`relay-os/workflows/code/with-self-review.md` — checking `gh pr view
...
1. **Lift sync-with-main into `code/open-pr`.** Move the mergeable-check /
   ...
   `relay-os/workflows/code/with-self-review.md` `## pr` so it relies on the
...
- `relay-os/skills/code/open-pr/SKILL.md` contains the sync-with-`main`
  step; `code/with-self-review.md` no longer duplicates it.
...
- `relay-os/skills/code/open-pr/SKILL.md` — shared PR-opening skill (target
  of change 1).
- `relay-os/workflows/code/with-self-review.md` — currently the only home of

...
2. **Committed-duplicate PR conflict.** A stranded ticket edit *committed on
   ...
   caught by open-pr's cleanliness gate (`open_pr.py:373-382`); committed ones
   are not.
...
- **Worktree enumeration.** `git worktree list --porcelain` is used in
  `branchsweep.py:310` and `branchcleanup.py:650` — those are the existing
  patterns to copy. Closest precedent for a *core* consumer is
...
- **`## Dev` parsers.** `parse_branch_name` (`autoclose.py:206`) and
  ...
  `startswith('(')`" (`open_pr.py:312-313`). `parse_worktree_path` consumers
  are the blast radius of any change to what `worktree:` means:
  `branchcleanup.py:133`, `open_pr.py:321` and `:589`, `commands/retire.py:280`,
  `commands/launch.py:2642`, `step_gate.py:59`, and `autoclose.py:420`.
  `parse_branch_name` additionally feeds `pr_assist.py:176` — a module that did
  not exist when this ticket was first written.
...
- **Single-checkout layout must keep working.** Where the primary checkout
  *is* the recorded worktree, "two copies" is one copy — the check must not
  report false divergence there. `open_pr._checkout_mode` (`open_pr.py:615`)

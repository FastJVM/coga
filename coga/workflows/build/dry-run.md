---
name: build/dry-run
description: Human-in-the-loop dry run of the coga build onboarding flow across empty / filled / ±CLAUDE.md repos. The agent role-plays `coga build` (not yet built), actually creates the starter tickets in throwaway fixture repos so the human can launch one or two, and the human scores each run against a rubric. Output: findings that feed the coga-build-onboarding-flow design.
steps:
...
— that's fine). Each must be a real coga repo so the dry run can actually
create and launch tickets in it:

1. **empty** — a fresh dir, `coga init`, nothing else.
2. **filled, no CLAUDE.md** — a dir with real-ish content (a README, some
   ...
   `coga init`. Make sure there is NO CLAUDE.md / AGENTS.md.
3. **filled, with CLAUDE.md** — same as #2 plus a representative CLAUDE.md (an
   agent guide describing the repo).

For each, after `coga init`: set `[notification.slack] enabled = false` in
that repo's `coga.local.toml` so `coga create` / `coga launch` work offline
...
Do NOT create tickets in this (coga-cli) repo — every ticket in this workflow
...
`coga build` end to end. The command does not exist yet — you are its
...
5. **Actually create a starter batch of tickets in that fixture repo** — cd
   into the fixture and use that repo's own `coga` to create them (drafts are
   fine; 3–6 is the target). Make at least one substantive enough that the
   human can `coga launch` it and watch it run.
...
answers what the `coga-build-onboarding-flow` design needs:
...
`marketing/coga-build-onboarding-flow` design step. Remind the human they can
now delete the fixture repos, then `coga mark done`.

#!/usr/bin/env python3
"""Headless phase of the phone-home period task; behavioral contract: coga/telemetry.

Ticket-owned, not a registered recipe: it runs the weekly snapshot under
the shared recipe-failure floor, then completes the step through the CLI.
Re-invoked as `ticket.py --worker <kind> <root>` for one bounded delivery.
"""
...
# An editable install from a source checkout is development, wherever this
# script was copied to.
_PACKAGE_FILE = Path(coga.__file__)
...
def _admitted(cfg: Config) -> bool:
    return (
        ...
        and not any(_development_tree(p) for p in (_PACKAGE_FILE, Path(__file__), cfg.repo_root, Path.cwd()))
    )
...
def _worker_main() -> None:
    ...
    try:
        cfg = load_config(Path(sys.argv[3]))
        if not _admitted(cfg):
            outcome = "suppressed"
        else:
            ...
            elif sys.argv[2] == "receipt" and set(data) == {"event", "outcome"} and data["outcome"] in _OUTCOMES:
...
def _bounded_worker(cfg: Config, kind: str, data: dict) -> str:
    ...
    try:
        child = subprocess.Popen(
            [sys.executable, str(Path(__file__).resolve()), "--worker", kind, str(cfg.repo_root)],
...
def main() -> int:
    code = run_reported(load_config(), "phone-home", run_phone_home, [])
    slug = os.environ.get("COGA_TASK_SLUG")
    if code or not slug:
        # Run by hand outside a launch: the report went to stdout, no step to close.
        return code
    ...
    return subprocess.run(
        [sys.executable, "-m", "coga.cli", "bump", slug],
        check=False,
    ).returncode

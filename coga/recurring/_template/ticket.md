...
assignee: replace-with-human-or-agent-type
...
# `coga recurring` snapshots them when it creates a period task, and a run
...
Run `coga recurring` directly (by hand or from an operator-owned scheduler) to
get-or-create the current period's task when this template's schedule is due.
Directories in

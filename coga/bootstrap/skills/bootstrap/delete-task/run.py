...
The single implementation of task deletion: the body behind `coga delete`, and
a standalone script skill. Works for both task shapes, keyed off the
unambiguous `COGA_TASK_TICKET`:
...
def main() -> int:
    raw = os.environ.get("COGA_TASK_TICKET")
    if not raw:
        return _fail("COGA_TASK_TICKET is not set")

    ticket = Path(raw)
    slug = os.environ.get("COGA_TASK_SLUG") or ticket.stem

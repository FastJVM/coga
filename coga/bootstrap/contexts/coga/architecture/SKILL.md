...
- **Workflows** are ordered step definitions. A repo's own workflows live in
  ...
  loop, `code/with-self-review`, `docs/create-google-doc`, the Dream child
...
interview skill. `coga recurring` scaffolding (a bare scan-and-launch run
and the on-demand `recurring launch <name>`, including the `coga dream`
alias) and `coga retire` scaffold their own one-shots straight to `active`

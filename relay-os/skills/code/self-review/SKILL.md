...
1. **Confirm state.** You should be on the task's branch with a clean
   working tree (the previous `code/implement` step committed). If
   not, `relay panic` — something is off.
...
3. **Capture findings on the blackboard** under a top-level
   `## Self-review` section. Verbatim is fine; don't summarize. Group as:

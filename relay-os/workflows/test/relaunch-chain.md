...
description: Synthetic probe for the relay launch auto-relaunch chain. Five steps deliberately arranged claude → claude → codex → human → claude so a single run exercises every boundary type — same-agent auto-relaunch, agent rotation, the human gate, and resumption after a human — while each step writes a section of a throwaway doc and logs what the supervisor actually did. Not a real delivery workflow; it exists to test relay itself.
steps:
  - name: draft
    assignee: agent
  - name: expand
...
The `relay launch` supervisor auto-relaunches the next step **only when the
...
| expand → peer-pass | claude → codex | **stop** — human must `relay launch` |
| peer-pass → human-check | codex → human | **stop** at the human gate |
| human-check → finalize | human → claude | **stop** — human must `relay launch` |
...
with a top heading `# Relay relaunch chain — test artifact` and a first section
...
blackboard logging protocol in the ticket's `## Context` and `relay bump`.

## expand

You are **claude** again, and if the chain works you were **auto-relaunched by
the supervisor** with no human `relay launch` in between — that is the single
...
far. Optionally append a `## 4. human-check (human)` note. Then `relay bump` to
hand to `finalize`. (You may also be the one running `relay launch` for the
...
then — this is the last step — run `relay mark done <slug>` instead of
`relay bump`.

...
`relay automerge`, run `relay mark done`, or otherwise advance/close the
...
merge. After the human merges, run `relay automerge` explicitly (or use
`relay mark done` if the PR is not recorded where automerge can read it)

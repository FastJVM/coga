...
description: Marketing-mode context for Relay launch/comms work. Loads Relay's positioning, audience, voice, proof points, and honest limits so an agent writing launch material stays on-message. Attach to any marketing/comms ticket. Source of truth is docs/market-thesis.md and docs/vision.md — when this drifts from them, they win.
---

# Relay marketing positioning

Domain facts for writing Relay's outward comms — launch posts, landing
copy, READMEs, talks, social. This is *what's true about Relay's market
...
Every piece of Relay comms hangs off one note: **own your machine,
...
- Category: **operations-as-code** — version-controlled, legible,
  agent-executable markdown for operational work. Relay is an engineered
  *runtime* for that discipline, not a convention and not an agent
  framework.
- The metaphor: **compile your company** — Relay refuses to let you stay
  vague; it makes you externalize tacit operational knowledge into
  explicit, editable artifacts. Relay's "epics and sprints."

## Audience — a narrow tribe, owned completely

Relay is **classical in a romantic stampede**: every competitor sells
"think less" (Viktor "a hire," Symphony "manage work not agents," Cowork
"set it once"); Relay sells "think more clearly, own the result."
...
- **But keep it sayable without the reading list.** The thesis leans on
  ...
  Relay is the apparatus that feeds it (contexts = facts, skills =
...
Relay owns no UI (it borrows the terminal, your editor, Slack), so don't
...
Comms must stay credible. State plainly where Relay is outclassed:
...
- **Linear**: the taste gold standard, but *subtractive* (felt as speed);
  Relay is *impositional* (felt as clarity). "Jira→Linear" is the proof
  that taste, not tech, wins this category.
- **Claude Code / Codex**: **not rivals — the engines Relay drives** and
...
- Product internals / how Relay works — `relay/architecture`,
  `relay/principles`.
